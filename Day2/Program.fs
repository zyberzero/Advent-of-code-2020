...
type PasswordRule = { Letter:char; minOccurs:int; maxOccurs:int }
type PasswordRow = { Rule:PasswordRule; Password:string; }
...
let parseRow (row:string):PasswordRow =
                   let parseRule (rule:string)  =
                       ...
                       {Letter=char; minOccurs=min; maxOccurs=max}
                   let parts = row.Split ':'
...
let verifyRowPart1 {Rule=rule;Password=password} =
     ...
     let chars = password.ToCharArray() |> List.ofSeq |> List.filter (fun c -> c = char) |> List.length
     chars >= min && chars <= max
...
let main argv =
    ...
    let calculateValidPasswords  verifier =  (input |> List.ofSeq |> List.map (fun l -> l |> parseRow |> verifier) |> List.filter id |> List.length)
